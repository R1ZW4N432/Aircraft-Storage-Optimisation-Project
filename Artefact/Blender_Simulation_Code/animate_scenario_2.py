...
# === CONFIG ===
SCENARIO_NAME = "Scenario 2 - Optimised Only"
CABIN_HEIGHT = 1.55
Z_LIFT_FRAMES = 20
ROTATE_FRAMES = 10
SLIDE_IN_FRAMES = 20
...
ROTATION_ANGLE_RAD = math.radians(-90)  # 90° Y-axis rotation

# === Get the collection for Scenario 2 ===
collection = bpy.data.collections.get(SCENARIO_NAME)
if not collection:
    raise ValueError(f"Collection '{SCENARIO_NAME}' not found.")

# === Filter all suitcases in the Scenario 2 collection ===
...
for index, suitcase in enumerate(sorted(suitcases, key=lambda o: o.location.x)):
    ...
    initial_rot = suitcase.rotation_euler.copy()
    ...
    # Step 3: Rotate 90° on Y-axis only
    suitcase.rotation_euler = (initial_rot.x, initial_rot.y + ROTATION_ANGLE_RAD, initial_rot.z)
